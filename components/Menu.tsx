...
const menuLinks: MenuLink[] = [
  ...
  },
  {
    name: "Discover",
    icon: (color) => (
      <FontAwesome6 name="earth-americas" size={22} color={color} />
    ),
    innerLink: [
...
const pagesLinks: MenuLink[] = [
  ...
  },
  {
    ...
    path: "/learn",
    icon: (color) => (
      <MaterialCommunityIcons name="chat-question" size={22} color={color} />
    ),
  },
  {
    name: "FAQs",
...
const Menu = () => {
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    transform: [
      { translateX: withTiming(menuOpen.value ? 0 : -400, { duration: 500 }) },
    ],
  ...
  return (
    <Animated.View
      className="justify-between h-screen w-[100%] absolute top-0 left-0 z-10"
      style={animatedStyle}
    >
      <TouchableWithoutFeedback
        onPress={() => {
                      ...
                      <View className="gap-6 pl-10 pt-6">
                        {link.innerLink?.map((l) => (
                          <Link href={l.path} key={l.name}>
                            <Text className="text-base text-[#878585e8]">
                              {l.name}
                            </Text>
                          </Link>
                        ))}
